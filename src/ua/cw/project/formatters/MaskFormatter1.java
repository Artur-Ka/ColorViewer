...
public class MaskFormatter1 extends DefaultFormatter
{
	 ...
	 */
	public MaskFormatter1(String mask) throws java.text.ParseException
	{
		this();
		setMask (mask);
	}
	...
	private String convertStringToValue(String value) throws ParseException
	{
		...
		for (int i = 0, j = 0; j < length; j++)
		{
			...
			if (i < value.length())
			{
				...
				{
					if (invalidChars != null && invalidChars.indexOf(valueChar) != -1)
			...
			{
				case NUM_CHAR:
					if (! Character.isDigit(valueChar)) throw new ParseException("Number expected: " + valueChar, i);
						result.append(valueChar);
					
					i++;
					break;
				case UPPERCASE_CHAR:
					if (! Character.isLetter(valueChar)) throw new ParseException("Letter expected", i);
				...
				case LOWERCASE_CHAR:
					...
					break;
				case ALPHANUM_CHAR:
					if (! Character.isLetterOrDigit(valueChar)) throw new ParseException("Letter or number expected", i);
						result.append(valueChar);
				...
				case LETTER_CHAR:
					...
					break;
				case HEX_CHAR:
					if (hexString.indexOf(valueChar) == -1 && ! isPlaceHolder) throw new ParseException("Hexadecimal character expected", i);
						result.append(valueChar);
				...
				case ESCAPE_CHAR:
					...
					if (j < length)
					{
						...
						if (! isPlaceHolder && getValueContainsLiteralCharacters() && valueChar != maskChar)
							throw new ParseException ("Invalid character: "+ valueChar, i);
	...
	private String convertValueToString(String value) throws ParseException
	{
		...
		for (int i = 0, j = 0; j < length; j++)
		{
			...
			{
				case NUM_CHAR:
					if ( ! isPlaceHolder && ! Character.isDigit(valueChar)) throw new ParseException("Number expected: " + valueChar, i);
						result.append(valueChar);
